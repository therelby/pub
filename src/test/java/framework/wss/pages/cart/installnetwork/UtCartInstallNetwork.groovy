...
//                browserVersionOffset: -1,   // use specific browser version
    ...
    def test() {
        ...
        testingQuery(installQueryProduct, address1.getZip())
        testingQuery(installQueryProduct, address2.getZip())

        for(userTypeForTesting in userTypesForTesting) {
            UserDetail newUser = new UserUrlLogin().loginReusableUser(userTypeForTesting, pbiNumber, address1, null, '_testing_install_network')
    ...
    void testingQuery(String itemNumber, String zipCode){
        assert (HpCartInstallNetwork.confirmProductZipCodeInstallNetwork(itemNumber, zipCode.substring(0, 5)).size() > 0)
    }
